...
use syn::{AngleBracketedGenericArguments, Attribute, Data, DataEnum, DataStruct, DeriveInput, Error, Expr, Field, Fields, GenericArgument, Ident, ItemEnum, ItemStruct, Meta, MetaList, NestedMeta, PathArguments, Stmt, Type, TypeArray, TypeParen, TypePath, TypeReference, TypeSlice, Variant, parse_macro_input, parse_quote};
...
pub fn derive_parse_from_item_stream(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    match try_derive_parse_from_item(input) {
        ...
        Err(mut err) => {
            err.combine(Error::new(Span::call_site(), "... while parsing this macro"));
...
impl FieldAttributeSpec {
    fn get_structr_attribute(attr: &Attribute) -> Result<Meta, syn::Error> {
        ...
        let ident = meta.path().get_ident().ok_or_else(|| Error::new(meta.span(), "Expected path to be identifier"))?;
    ...
    fn get_structr_list(meta: Meta) -> Result<MetaList, syn::Error> {
        match meta {
            ...
            _ => Err(Error::new(meta.span(), "Expected structr attribute to be a list: #[structr(like, this)]"))
        }
    }

    fn parse_attrs(attrs: &[Attribute]) -> Result<Self, syn::Error> {
        let mut field_structr_attrs = Self::default();
        for structr_meta in attrs.iter().map(Self::get_structr_attribute).filter_map(Result::ok) {
            let list = Self::get_structr_list(structr_meta)?;
            for item in list.nested {
                let nested_meta = match item {
                    NestedMeta::Meta(meta) => meta,
                    _ => return Err(Error::new(item.span(), "Unexpected string literal in structr attribute"))
                };
                let name_value = match nested_meta {
                    Meta::NameValue(name_value) => name_value,
                    _ => return Err(Error::new(nested_meta.span(), "Expected structr(name = value)"))
                };
                let ident = name_value.path.get_ident().ok_or_else(|| Error::new(name_value.span(), "Unexpected name to be path in structr(name = value)"))?;
                let get_str_lit = || {
                    match &name_value.lit {
                        syn::Lit::Str(str) => Ok(str),
                        _ => return Err(Error::new(ident.span(), format!("Expected structr \"{}\" attribute to have string literal as value", ident.to_string())))
                    }
                };
                match ident.to_string().as_ref() {
                    ...
                    "hidden" => field_structr_attrs.hidden = Some(get_str_lit()?.parse_with(Field::parse_named)?),
                    "parse" => {
                        // let mut value: ExprCall = ;
                        // value.args.insert(0, parse_quote!{ parser });
                        field_structr_attrs.parse = Some(get_str_lit()?.parse()?);
                    },
                    other => return Err(Error::new(ident.span(), format!("Unexpected structr attribute {}", other)))
                }
            }
        }
...
impl From<DeriveInput> for DeriveInputItem {
    fn from(value: DeriveInput) -> Self {
        let DeriveInput { attrs, vis, ident, generics, data } = value;
        match data {
            Data::Struct(DataStruct { struct_token, fields, semi_token }) => {
                DeriveInputItem::Struct(ItemStruct { attrs, fields, generics, ident, semi_token, struct_token, vis })
            },
            Data::Enum(DataEnum { enum_token, brace_token, variants }) => {
                DeriveInputItem::Enum(ItemEnum { attrs, vis, enum_token, ident, generics, brace_token, variants })
            },
            Data::Union(_) => {
                unimplemented!()
            }
...
fn get_type_ident(type_: &Type) -> String {
    match type_ {
        Type::Array(TypeArray { elem, len, .. }) => { format!("[{}; {}]", elem.to_token_stream(), len.to_token_stream()) }
                    ...
                    PathArguments::AngleBracketed(AngleBracketedGenericArguments {
                        ...
                        for arg in args.iter() {
                            match arg {
                                ...
                                GenericArgument::Lifetime(lifetime) => { s += &format!("&{}", lifetime.ident.to_token_stream()) }
        ...
        Type::Reference(TypeReference { and_token, elem, mutability, .. }) => {
            format!("{}{}{}", and_token.to_token_stream(), if let Some(mutability) = mutability {
                mutability.to_token_stream().to_string() + " "
            } else {"".to_string()}, elem.to_token_stream())
        }
        Type::Slice(TypeSlice { elem, .. }) => { format!("[{}]", elem.to_token_stream()) }
        Type::Tuple(_) => { "unknown".to_owned() }
        _ => "unknown".to_owned()
    }
}

fn derive_field_value(FieldInfo { field: Field { ty, .. }, some_ident: ident, attrs, ..}: &FieldInfo) -> Result<TokenStream, Error> {
...
fn derive_field_assignment(FieldInfo { field, attrs, some_ident, ..}: &FieldInfo) -> Result<TokenStream, Error> {
...
fn derive_struct(struct_input: ItemStruct) -> Result<TokenStream, Error> {
    ...
    let field_info = struct_input.fields.iter().enumerate().map(|(i, field)| -> Result<FieldInfo, Error> {
        ...
        Ok(FieldInfo { attrs, field, some_ident, _field_index: i, _fields: &struct_input.fields, _debugging: false, _hidden: false, _binding: false })
    }).collect::<Result<Vec<FieldInfo>, Error>>()?;
    let field_assignment = field_info.iter().map(derive_field_assignment).collect::<Result<Vec<_>, _>>()?;
    let field_parsing = field_info.iter().map(derive_field_value).collect::<Result<Vec<_>, _>>()?;
...
fn generate_variants_only_enum(complete: &ItemEnum) -> Result<(ItemEnum, TokenStream), Error> {
    ...
    for Variant { ref mut fields, ident, .. } in variants_only.variants.iter_mut() {
...
fn derive_enum(enum_: ItemEnum) -> Result<TokenStream, Error> {
    ...
    if let Some((type_, expr_variant_map)) = try_get_discriminants(&enum_) {
        let variant_parse = expr_variant_map.iter().map(|(expr, ident)| {
        ...
        let variant_parse = enum_.variants.iter().map(|variant| {
            // let field = variant.fields.iter().next().unwrap();
